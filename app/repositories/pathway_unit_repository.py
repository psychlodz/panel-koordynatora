    ...
    with closing(create_local_connection()) as connection:
        with connection:
            connection.execute(
                ...
                VALUES (?, ?, ?, ?)
                ON CONFLICT(sciezka_id, jo_id) DO UPDATE SET
    ...
    with closing(create_local_connection()) as connection:
        with connection:
            cursor = connection.execute(
                """
                DELETE FROM pk_pathway_units
                WHERE sciezka_id = ?
                  AND jo_id = ?
                """,
                (sciezka_id, _unit_id(jo_id)),
            )
            if cursor.rowcount == 0:
                raise ValueError("Jednostka nie była przypisana ścieżce")
