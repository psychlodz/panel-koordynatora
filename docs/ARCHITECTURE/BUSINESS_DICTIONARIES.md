...
klocek może odpowiadać wielu kodom rodzajów wizyt. Jeden rodzaj wizyty może
mieć tylko jedno aktywne przypisanie do klocka. Źródłem prawdy pozostaje
widok Oracle `V_KOMPAS_PARAMETRY_WIZYT`; PostgreSQL przechowuje lokalny
słownik referencyjny i przypisania administracyjne.
...
CREATE UNIQUE INDEX uq_pk_mapowanie_wizyt_active_rodzaj
ON pk_mapowanie_wizyt(rodzaj_wizyty_id)
WHERE czy_aktywne = 1;
```

Źródłem prawdy dla kodu, nazwy i aktualności rodzaju wizyty pozostaje Oracle.
PostgreSQL przechowuje lokalny słownik referencyjny
`pk_rodzaje_wizyt_eskulap` oraz przypisania do klocków. Jeden klocek może mieć
wiele aktywnych rodzajów wizyt, ale jeden rodzaj wizyty może mieć tylko jedno
aktywne przypisanie do klocka.

```mermaid
flowchart LR
    W["WP_PARAMETR<br/>Oracle"]
    R[("pk_rodzaje_wizyt_eskulap")]
    M[("pk_mapowanie_wizyt")]
    K[("pk_klocki")]
    E[("pk_sciezka_elementy")]
    Z[("pk_zadania")]

    W -->|"synchronizacja SELECT"| R
    K -->|"1:N"| M
    R -->|"1:N"| M
    K --> E
    E --> Z
...
| `PKK_KWAL` | `F35` | Dodatkowy rodzaj wizyty kwalifikacyjnej |
| `PKK_KWAL` | `F99` | Dodatkowy rodzaj wizyty kwalifikacyjnej |
