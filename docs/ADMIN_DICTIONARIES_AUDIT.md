...
| Filtr wizyty kwalifikacyjnej PKK | Kod integracyjny `F18` w jednym miejscu | `app/repositories/qualification_repository.py`: `PKK_KWAL_PARAMETR_KOD` | Docelowo kontrolowana | `F18` jest kodem `WP_PARAMETR` wizyty kwalifikacyjnej PKK, opisanym w `CG_REF_CODES`. Ewentualną przyszłą konfigurację należy udostępnić z ekranem testowego podglądu wyników. |
