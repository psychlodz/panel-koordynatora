...
| Mapowanie klocków na rodzaje wizyt Eskulapa | Tabela `pk_mapowanie_wizyt`; słownik źródłowy pozostaje w Oracle | `db/postgres/003_schema.sql`, `app/repositories/visit_mapping_repository.py`, `app/ui/visit_mappings_widget.py` | Pełna edycja przypisań | Administrator przypisuje wiele kodów `WP_PARAMETR` do jednego klocka. Jeden rodzaj wizyty może mieć tylko jedno aktywne przypisanie. Nie edytuje kodów ani nazw pochodzących z Oracle. |
| Filtr wizyty kwalifikacyjnej PKK | Aktywne mapowania klocka `PKK_KWAL`; seed zawiera `F18` | `pk_mapowanie_wizyt`, `app/repositories/qualification_repository.py` | Kontrolowana | Filtr jest budowany z mapowań bazodanowych. Dodanie kolejnego rodzaju kwalifikacji nie wymaga zmiany kodu aplikacji. |
